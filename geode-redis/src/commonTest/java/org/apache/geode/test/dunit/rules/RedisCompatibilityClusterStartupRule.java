...
public class RedisCompatibilityClusterStartupRule extends ClusterStartupRule {

  public RedisCompatibilityClusterStartupRule() {
    super();
  }

  public RedisCompatibilityClusterStartupRule(int numVMs) {
    super(numVMs);
  }
  ...
  private ServerStarterRule withRedis(ServerStarterRule rule) {
    return rule.withProperty(REDIS_BIND_ADDRESS, "localhost")
        ...
        .withSystemProperty(RedisCompatibilityServer.ENABLE_REDIS_UNSUPPORTED_COMMANDS_PARAM,
            "true");
  }
  ...
  public int getRedisPort(MemberVM vm) {
    return vm.invoke(() -> {
      RedisCompatibilityService service =
          ClusterStartupRule.getCache().getService(RedisCompatibilityService.class);
