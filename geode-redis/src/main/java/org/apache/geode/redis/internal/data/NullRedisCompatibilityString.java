...
import org.apache.geode.redis.internal.executor.string.RedisCompatibilityStringCommands;
import org.apache.geode.redis.internal.executor.string.RedisCompatibilityStringCommandsFunctionInvoker;
 ...
 * to be modified.
 */
public class NullRedisCompatibilityString extends RedisCompatibilityString {
  public NullRedisCompatibilityString() {
  ...
  @Override
  public int bitpos(Region<ByteArrayWrapper, RedisCompatibilityData> region, ByteArrayWrapper key,
      int bit,
  ...
  public int setbit(
    ...
    RedisCompatibilityString newValue;
    if (bitValue == 1) {
      byte[] bytes = new byte[byteIndex + 1];
      bytes[byteIndex] = (byte) (0x80 >> bitIndex);
      newValue = new RedisCompatibilityString(new ByteArrayWrapper(bytes));
    } else {
      // all bits are 0 so use an empty byte array
      newValue = new RedisCompatibilityString(new ByteArrayWrapper(new byte[0]));
    }
  ...
  @Override
  public long incr(Region<ByteArrayWrapper, RedisCompatibilityData> region, ByteArrayWrapper key)
      throws NumberFormatException, ArithmeticException {
    byte[] newValue = {Coder.NUMBER_1_BYTE};
    region.put(key, new RedisCompatibilityString(new ByteArrayWrapper(newValue)));
    return 1;
  }

  @Override
  public long incrby(Region<ByteArrayWrapper, RedisCompatibilityData> region, ByteArrayWrapper key,
  ...
  @Override
  public BigDecimal incrbyfloat(Region<ByteArrayWrapper, RedisCompatibilityData> region,
  ...
  @Override
  public long decr(Region<ByteArrayWrapper, RedisCompatibilityData> region, ByteArrayWrapper key)
      throws NumberFormatException, ArithmeticException {
    region.put(key, new RedisCompatibilityString(new ByteArrayWrapper(Coder.stringToBytes("-1"))));
    return -1;
  }

  @Override
  public long decrby(Region<ByteArrayWrapper, RedisCompatibilityData> region, ByteArrayWrapper key,
      long decrement) {
  ...
  public int append(ByteArrayWrapper appendValue,
      Region<ByteArrayWrapper, RedisCompatibilityData> region,
      ByteArrayWrapper key) {
    region.put(key, new RedisCompatibilityString(appendValue));
    return appendValue.length();
  }

  @Override
  public ByteArrayWrapper getset(Region<ByteArrayWrapper, RedisCompatibilityData> region,
      ByteArrayWrapper key,
      ByteArrayWrapper value) {
    region.put(key, new RedisCompatibilityString(value));
    return null;
  }

  @Override
  public int setrange(Region<ByteArrayWrapper, RedisCompatibilityData> region, ByteArrayWrapper key,
      int offset,
      byte[] valueToAdd) {
    ...
    if (valueToAdd.length != 0) {
      ...
      region.put(key, new RedisCompatibilityString(new ByteArrayWrapper(newBytes)));
    }
  ...
  public boolean set(CommandHelper helper, ByteArrayWrapper key,
    ...
    RedisCompatibilityString redisCompatibilityString = helper.setRedisString(key, value);
    redisCompatibilityString.handleSetExpiration(options);
    return true;
  }
  ...
  private boolean setnx(CommandHelper helper, ByteArrayWrapper key,
    ...
    RedisCompatibilityString redisCompatibilityString = new RedisCompatibilityString(value);
    redisCompatibilityString.handleSetExpiration(options);
    helper.getRegion().put(key, redisCompatibilityString);
    return true;
  }
  ...
  public int bitop(CommandHelper helper,
    ...
    RedisCompatibilityStringCommands commander =
        new RedisCompatibilityStringCommandsFunctionInvoker(helper.getRegion());
  ...
  private int doBitOp(CommandHelper helper,
    ...
    if (selfIndex != -1) {
      RedisCompatibilityString redisCompatibilityString = helper.getRedisString(key, true);
      if (!redisCompatibilityString.isNull()) {
        sourceValues.set(selfIndex, redisCompatibilityString.getValue());
      }
    }
    int maxLength = 0;
